...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = events.size()-1; i>=0; i--){
            ...
            img.setId(i);
            img.setImageBitmap(BitmapFactory.decodeByteArray(events.get(i).getPicture(), 0, events.get(i).getPicture().length));
    ...
    public void action(View view){
       ...
       /* events.get(i).getPicture().compress(Bitmap.CompressFormat.PNG, 100, bStream);
        byte[] byteArray = bStream.toByteArray();*/
        intent.putExtra("picture", events.get(i).getPicture());
    ...
    private void createEvent(){
       ...
       */ if(name != null && date != null && hour != null && duration != null && description != null && location != null && picture != null) {
            if (link==null)
    ...
    public List<Event> load(Context context) {
        ...
        if (settings.contains("events")) {
            ...
            Gson gson = new Gson();
            Event[] eventsArr = gson.fromJson(jsonFavorites, Event[].class);
